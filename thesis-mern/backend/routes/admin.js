...
router.post('/assign', protect, requireRole('admin'), async (req, res) => {
  try {
    ...
    // Check if paper already has an active assignment (Assigned or Requested)
    const alreadyAssigned = await Assignment.findOne({ paper_id, status: { $in: ['Assigned'] } });
    if (alreadyAssigned) {
      return res.status(400).json({ message: 'This paper already has an active assignment' });
    }

    // If there's an existing request by the author, update it to assign the reviewer
    const existingRequest = await Assignment.findOne({ paper_id, status: 'Requested' });
    let assignment;
    if (existingRequest) {
      existingRequest.reviewer_id = reviewer_id;
      existingRequest.status = 'Assigned';
      existingRequest.assigned_date = new Date();
      assignment = await existingRequest.save();
    } else {

...
// ── POST /api/papers/request-reviewer ───────────────────────
// Author requests a reviewer to be assigned by admin (creates a request without reviewer_id)
router.post('/request-reviewer', protect, requireRole('author'), async (req, res) => {
  try {
    const { paper_id } = req.body;
    if (!paper_id) return res.status(400).json({ message: 'Paper ID is required' });

    const paper = await Paper.findById(paper_id);
    if (!paper) return res.status(404).json({ message: 'Paper not found' });
    if (paper.author_id.toString() !== req.user._id.toString()) {
      return res.status(403).json({ message: 'You do not own this paper' });
    }

    // Prevent duplicate requests or existing assignments
    const Assignment = require('../models/Assignment').Assignment;
    // Debug: log enum values loaded in the Assignment schema
    try {
      console.log('Assignment.status enumValues at runtime:', Assignment.schema.paths.status.enumValues);
    } catch (e) {
      console.log('Could not read Assignment.schema.paths.status.enumValues', e.message);
    }

    const existing = await Assignment.findOne({ paper_id, status: { $in: ['Requested','Assigned'] } });
    if (existing) return res.status(400).json({ message: 'A request or assignment already exists for this paper' });

    const request = await Assignment.create({ paper_id, reviewer_id: null, assigned_date: null, status: 'Requested' });

    res.status(201).json({ message: 'Reviewer request created successfully', request });
  } catch (err) {
    console.error('Request reviewer error:', err);
    res.status(500).json({ message: 'Error creating reviewer request' });
  }
});

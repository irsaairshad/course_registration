...
export default function ReviewerPanel() {
  ...
  const handleDownload = async (paperId, filePath, paperTitle) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', filePath || `${paperTitle}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
    } catch (err) {
      console.error('Download error:', err);
      alert('Failed to download paper. Please check if the file exists.');
    }
  };
            ...
            <div style={{display:'flex', justifyContent:'space-between', flexWrap:'wrap', gap:'1rem'}}>
              <div>
                ...
                <div style={{display:'flex', gap:'10px', marginTop:'0.8rem', flexWrap:'wrap'}}>
                  <Link to={`/view-paper/${a.paper_id?._id}`} className="btn btn-secondary btn-sm" style={{textDecoration:'none', display:'inline-flex', alignItems:'center'}}>
                    📄 View Details
                  </Link>
                  <button 
                    onClick={() => handleDownload(a.paper_id?._id, a.paper_id?.file_path, a.paper_id?.title)} 

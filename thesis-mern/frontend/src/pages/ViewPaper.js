...
export default function ViewPaper() {
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(`/api/papers/download/${paper._id}`, {
        responseType: 'blob'
      });
      const url = window.URL.createObjectURL(new Blob([response.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', paper.file_path || `${paper.title}.pdf`);
      document.body.appendChild(link);
      link.click();
      link.parentNode.removeChild(link);
    } catch (err) {
      console.error('Download error:', err);
      alert('Failed to download paper. You may not have access or the file does not exist.');
    }
  };
